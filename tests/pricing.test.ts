...
/**
 * Релевантність на живому каталозі.
 *
 * Ці приклади не вигадані — усі зняті з прогону сценарію «тірамісу» на
 * справжньому MCP «Сільпо». Пошук працює за схожістю, тому «Маскарпоне»
 * приводило «Тістечко Макарон», а «Яйця» — «Молоко пастеризоване», і молоко
 * ставало БЮДЖЕТНИМ рівнем, бо було найдешевшим.
 */
describe('товар мусить бути самим інгредієнтом', () => {
  it('відсіює зовсім інший продукт, навіть без заборонених слів у назві', () => {
    expect(isPlausibleIngredientMatch('Молоко пастеризоване «Ферма» 3,2%', 'Яйця')).toBe(false)
    expect(isPlausibleIngredientMatch('Тістечко Макарон Дорблю та груша', 'Маскарпоне')).toBe(false)
  })

  it('лишає товар, назва якого містить інгредієнт', () => {
    // ключ товару — «сир», а не «маскарпоне»: сам нормалізатор тут не допоміг би
    expect(isPlausibleIngredientMatch('Сир Ghidetti «Маскарпоне» 45%', 'Маскарпоне')).toBe(true)
    expect(isPlausibleIngredientMatch('Печиво «Премія»® Савоярді', 'Печиво савоярді')).toBe(true)
  })

  it('лишає товар, ключ якого збігається з ключем інгредієнта', () => {
    expect(isPlausibleIngredientMatch('Яйця курячі Свіжак харчові столові C2', 'Яйця')).toBe(true)
    expect(isPlausibleIngredientMatch('Какао-порошок «Добрик» темний', 'Какао')).toBe(true)
    expect(isPlausibleIngredientMatch('Кава мелена Buco Рецепт Кенії', 'Кава мелена')).toBe(true)
  })

  it('кондитерська форма відсіюється навіть за збігу назви', () => {
    expect(isPlausibleIngredientMatch('Яйце шоколадне The Smurfs із сюрпризом', 'Яйця')).toBe(false)
  })
})
