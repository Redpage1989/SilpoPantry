#!/usr/bin/env python3
"""Змонтувати кліп «Де придбали»: посценна озвучка лягає на початок своєї сцени.

  python3 scripts/mux-clip-place.py   →  tutorial-out/clip-place-voiced.mp4

Сцени — з docs/clip-place-scenes.json (записані record-clip-place.ts),
голос — tutorial-out/clip-vo/NN.mp3. Кожна репліка стартує рівно на межі
...
V = OUT / 'clip-place.webm'
VO = sorted((OUT / 'clip-vo').glob('*.mp3'))
scenes = json.loads(pathlib.Path('docs/clip-place-scenes.json').read_text())
...
target = OUT / 'clip-place-voiced.mp4'
