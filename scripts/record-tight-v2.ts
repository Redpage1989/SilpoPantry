...
const TARGETS: number[] = readdirSync('tutorial-out/demo-v2-vo')
  .filter((f) => f.endsWith('.mp3'))
  .sort()
  .map((f) => Number(execFileSync('ffprobe', ['-v', 'error', '-show_entries', 'format=duration', '-of', 'csv=p=0', join('tutorial-out/demo-v2-vo', f)]).toString()) + GAP)
...
async function main() {
  ...
  /**
   * Сцени «спільнота» тут більше немає.
   *
   * Вона показувала стрічку на дванадцять секунд і встигала сказати рівно
   * те, що тепер займає окремий фрагмент: рецепти від родин і голосування.
   * Гірше — запис застарів: на ньому два рецепти авторства самого демо й
   * нуль голосів, тобто картина, якої в застосунку вже немає.
   *
   * Звільнені секунди пішли під фрагмент про зароблений голос і таблицю
   * тижня — ролик мусить лишатися в межах пʼяти хвилин.
   */
  ...
  const webm = readdirSync(OUT)
    ...
    .sort((a, b) => b.t - a.t)[0]
  if (webm) renameSync(join(OUT, webm.f), join(OUT, 'demo-v2.webm'))
  writeFileSync('docs/demo-v2-scenes.json', JSON.stringify({ total, scenes: marks }, null, 2))

  console.log('\n  #    старт  сцена')
  marks.forEach((m) => console.log(`${(m.i + 1).toString().padStart(3)} ${m.start.toFixed(2).padStart(8)}с ${m.label}`))
  console.log(`\nвсього: ${total.toFixed(1)} с`)
  if (over.length) {
