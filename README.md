...
```bash
npm run verify            # typecheck + lint + 84 unit-тести
...
| Можливість | Стан за замовчуванням | Що вмикає live |
|---|---|---|
| Каталог, кошик, чеки, лояльність, слоти доставки | **DEMO** (seed-дані) | «Увійти через «Сільпо»» → OAuth 2.1 + PKCE |
| `tools/list` на `https://mcp.silpo.ua/mcp` | **DEMO** (емуляція) | той самий вхід; результат видно на `/trace` |
| Розпізнавання фото | **DEMO** (детермінований аналізатор) | `ANTHROPIC_API_KEY` у `.env` |
| Комора, рецепти, скоринг, списання, порівняння «готувати vs купити» | **Працює завжди** | — |
