 ...
 *
 * Експортовано не заради зручності, а заради тесту-сторожа: КОЖЕН інгредієнт
 * звідси мусить бути канонічним ключем словника. Продукт, якого словник не
 * знає, лишається нерозпізнаним — а нерозпізнаний склад не дає модерації
 * побачити алерген, і рецепт із мигдалем публікується без декларації
 * «горіхи». Саме так шістнадцять носіїв алергенів були невидимі.
 */
export const ALLERGEN_SOURCES: Record<string, string[]> = {
