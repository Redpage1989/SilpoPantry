...
3. **Транспортні збої та шлюзові 502** — приблизно один запит із десятка падає
   до відповіді. Клієнт ретраїть транспортні помилки і 502/503/504 (відповів
   балансувальник, запит до MCP не дійшов), але ніколи — 4xx і 500.
4. **Кошик читається двома викликами**: `get_my_shopping_cart` повертає лише
   `shoppingCartId`, вміст дає `get_shopping_cart_by_id`. Товари лежать
   у `cart.shipments[].products[]`, підсумки — у `cart.calculation`.
5. **Каталог не повертає ваги упаковки** — тільки `weighted` і `step`. Тому
   вага береться з назви товару, а якщо її там немає, показуємо «1 упаковка»
   замість вигаданих «1 шт».
6. **Пошук за точною фразою**: «тірамісу» знаходить 5 товарів,
   «тірамісу готовий десерт кулінарія» — жодного.

> **Пастка, на яку тут наступали двічі:** `\b` у регулярних виразах працює за
> ASCII, тому `/\bторт/` **ніколи** не збігається з кирилицею. Через це мовчки
> не спрацьовували і парсер ваги, і евристика порцій. У цьому проєкті для
> української використовуємо `includes` або явні lookahead-межі.
...
| `generateWeeklyPlan` | read | домейн (симуляція споживання) |
| `saveMealPlan` | **write** | БД, потребує підтвердження |

## 6a. Тижневий раціон: чому це не сім разів «підбери страву»

Наївний план скорить рецепти сім разів поспіль — і отримує сім однакових
вечерь, бо кожен день «бачить» ту саму повну комору.

```mermaid
flowchart TD
  S([Старт: комора на понеділок]) --> D1[День 1: сніданок/обід/вечеря]
  D1 --> C1["planDeduction:\nсписати спожите"]
  C1 --> P1[(Комора після дня 1)]
  P1 --> D2[День 2]
  D2 --> C2[planDeduction]
  C2 --> P2[(Комора після дня 2)]
  P2 --> DN[…]
  DN --> AGG["Звести нестачу:\nодин продукт = одна позиція"]
  AGG --> LIST[Список покупок на тиждень]
```

Три властивості, які з цього випливають:

1. **Список покупок чесний** — молоко з понеділка не «покриває» ще й четвер.
2. **Продукти, що псуються, ідуть у перші дні** (`rescueMode` для днів 0–1):
   далі вони просто зіпсуються, і планувати на них не можна.
3. **Бюджет — обмеження, а не фактор із вагою 0,10.** Він витрачається
   наростаючим підсумком; коли вичерпаний, агент переходить на страви
   з мінімальною докупівлею замість того, щоб мовчки виставити рахунок.

Плюс вікно різноманіття (`VARIETY_WINDOW_DAYS = 3`) і різний ліміт часу
для буднів (35 хв) і вихідних (90 хв).
...
erDiagram
  ...
  User ||--o{ UserRecipe : публікує
  User ||--o{ RecipeVote : голосує
  User ||--o{ WeeklyAward : виграє
  HouseholdMember ||--o{ FoodRestriction : стосується
  Recipe ||--o{ MealPlan : використовується
  UserRecipe ||--o{ RecipeVote : збирає
  UserRecipe ||--o{ WeeklyAward : нагороджується
```

Три обмеження в цій частині схеми несуть логіку, а не лише порядок:

`RecipeVote` унікальний за `(userRecipeId, voterId, isoWeek)`. Гарантує це база, а
не код: без такого обмеження достатньо двох паралельних натискань, щоб одна
людина віддала два голоси.

`WeeklyAward.isoWeek` унікальний. Переможець тижня фіксується один раз і не
перераховується — пізніші голоси не мають переписувати вже оголошений результат.
Ця ж унікальність робить закриття тижня ідемпотентним: воно викликається під час
читання стрічки, і паралельні читачі не створять двох переможців.

`UserRecipe.compositionVerified` — межа довіри до складу. У наших 30 рецептах
`normalizedName` виставлений вручну й вивірений; у користувацьких він виводиться
з вільного тексту й помиляється. Рецепт із нерозпізнаним інгредієнтом видимий у
стрічці, але не потрапляє в скоринг і планування (див. §9).
...
| Склад користувацького рецепта не завжди розпізнається | рецепт позначається неперевіреним і **виключається з підбору страв**; алергени декларує автор зі закритого списку |
| Приз за рецепт тижня неможливо нарахувати | у MCP немає запису лояльності; формуємо заявку зі статусом «очікує» і пишемо це в інтерфейсі |
| Рецепти спільноти не модеруються | прийнятно для прототипу; для пілоту потрібна черга модерації |
